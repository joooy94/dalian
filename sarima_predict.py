...
def predict_with_95ci(model, input_data, points_per_day, output_dir, prediction_date=None):
    """
    使用SARIMA模型预测未来一天的数据，包含95%置信区间
    
    参数:
        model: 训练好的SARIMA模型
        input_data: 输入数据（最近几天的数据）
        points_per_day: 每天的数据点数
        output_dir: 输出目录
        prediction_date: 预测日期，如果为None则使用输入数据的最后一天的下一天
    """
    print("\n预测未来一天数据（含95%置信区间）...")
    ...
    try:
        ...
        if prediction_date is None:
            ...
            start_date = last_date + timedelta(minutes=30)  # 从最后一个数据点后开始预测
        else:
            start_date = pd.to_datetime(prediction_date)
        
        # 创建预测日期范围
        future_dates = pd.date_range(start=start_date, periods=points_per_day, freq=f'30T')
        ...
        result_df = pd.DataFrame({
            ...
            'lower_ci': forecast_ci.iloc[:, 0].values,  # 下界
            'upper_ci': forecast_ci.iloc[:, 1].values   # 上界
        }, index=future_dates)
        
        # 保存到CSV
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_data_to_csv(result_df, f'prediction_{timestamp}.csv', output_dir)
        
        # 打印置信区间信息
        print("\n95%置信区间样例:")
        sample_point = result_df.iloc[0]
        print(f"预测值: {sample_point['predicted']:.2f}")
        print(f"95%置信区间: [{sample_point['lower_ci']:.2f}, {sample_point['upper_ci']:.2f}]")
        print(f"区间宽度: {sample_point['upper_ci'] - sample_point['lower_ci']:.2f}")
        ...
        try:
            ...
            # 使用残差标准差构建95%置信区间
            z_value = 1.96  # 对应95%置信区间
            print(f"使用z值: {z_value} (对应95%置信区间)")
            ...
            # 保存到CSV
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            save_data_to_csv(result_df, f'prediction_approx_{timestamp}.csv', output_dir)
...
def main():
    ...
    # 加载输入数据
    input_data = load_input_data(args.input_file, args.date_column, args.target_column, sampling_interval)
